...
export function CatalystFeed({ items, loading }: Props) {
  return (
    <aside className="flex h-full w-80 shrink-0 flex-col border-l border-slate-800 bg-slate-900">
      <div className="flex h-14 shrink-0 items-center gap-2 border-b border-slate-800 px-3">
        ...
        <div>
          ...
          <div className="text-[10px] text-slate-500">Cross-asset impact timeline</div>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {loading && <p className="px-3 py-4 text-xs italic text-slate-500">Loading…</p>}
